...
exports.run = async (client, message, args) => {
  ...
  if (opponent.id === message.author.id) return message.reply('Kendin ile Yıldız Savaşı Atamazsın');
		...
		try {
			if (!opponent.bot) {
                await message.channel.send(`${opponent}, Yıldız Savaşı isteği geldi. Yıldız Savaşını kabul ediyor musun? (\`evet\` veya \`hayir\` olarak cevap veriniz.)`);
